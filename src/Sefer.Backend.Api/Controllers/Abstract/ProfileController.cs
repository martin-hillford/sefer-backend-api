...
public abstract class ProfileController(IServiceProvider serviceProvider, IPasswordService passwordService) : UserController(serviceProvider)
{
    ...
    protected async Task<ActionResult> UpdateProfileInformation(IHttpContextAccessor httpContextAccessor, int userId)
    {
        ...
        var validPassword = passwordService.IsValidPassword(user, profile.Password);
        if (validPassword == false) return Forbid();

        // Check if the changes are valid and if the email is unique (400)
        ...
        var statusCode = 200;
        var newEmail = profile.Email.ToLowerInvariant().Trim();
        if (newEmail != user.Email.ToLowerInvariant().Trim())
        {
