...
[TestClass]
public class GetClosedCourseRevisionsHandlerTest : HandlerUnitTest
{
    [TestInitialize]
    ...
    {
        var request = new GetClosedCourseRevisionsRequest(1);
        var handler = new GetClosedCourseRevisionsHandler(GetServiceProvider().Object);
        var result = await handler.Handle(request, CancellationToken.None);
        
        Assert.AreEqual(2, result.Count);
        Assert.IsTrue(result.Any(r => r.Version == 1));
        Assert.IsTrue(result.Any(r => r.Version == 3));
    }
}